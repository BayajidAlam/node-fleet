...
export const scalingFailureAlarm = new aws.cloudwatch.MetricAlarm("scaling-failure-alarm", {
...
export const cpuOverloadAlarm = new aws.cloudwatch.MetricAlarm("cpu-overload-alarm", {
  ...
  statistic: "Average",
  threshold: 90,
  alarmDescription: "Alert when cluster CPU exceeds 90% for 5+ minutes (capacity exhausted)",
  alarmActions: [slackTopic.arn],
  treatMissingData: "notBreaching",
});

// 3. At Max Capacity Alarm - Node count = 10 for 10+ minutes
export const maxCapacityAlarm = new aws.cloudwatch.MetricAlarm("max-capacity-alarm", {
  ...
  statistic: "Average",
  threshold: 10,
  alarmDescription: "Alert when node count reaches maximum (10) for 10+ minutes - cannot scale further",
  alarmActions: [slackTopic.arn],
  treatMissingData: "notBreaching",
});

// 4. Node Join Failure Alarm - New instance not Ready after 5 minutes
export const nodeJoinFailureAlarm = new aws.cloudwatch.MetricAlarm("node-join-failure-alarm", {
  ...
  alarmDescription: "Alert when new node takes longer than 5 minutes to become Ready",
  alarmActions: [slackTopic.arn],
  treatMissingData: "notBreaching",
});
...
export const highMemoryAlarm = new aws.cloudwatch.MetricAlarm("high-memory-alarm", {
  ...
  statistic: "Average",
  threshold: 80,
  alarmDescription: "Warning: Cluster memory utilization exceeds 80% for 10 minutes",
  alarmActions: [slackTopic.arn],
  treatMissingData: "notBreaching",
});

// 6. Lambda Timeout Warning - Execution time > 50 seconds
export const lambdaTimeoutAlarm = new aws.cloudwatch.MetricAlarm("lambda-timeout-alarm", {
  ...
  alarmDescription: "Warning: Lambda execution time approaching 60s timeout limit",
  alarmActions: [slackTopic.arn],
  treatMissingData: "notBreaching",
  dimensions: {
...
export const pendingPodsAlarm = new aws.cloudwatch.MetricAlarm("pending-pods-alarm", {
  ...
  statistic: "Average",
  threshold: 0,
  alarmDescription: "Alert when pods remain pending for 6+ minutes (autoscaler may not be responding)",
  alarmActions: [slackTopic.arn],
  treatMissingData: "notBreaching",
});
